...
export const moderatorRelationships = pgTable(
  ...
  (table) => {
    return {
      streamerModeratorIndex: uniqueIndex('streamer_moderator_idx').on(
        table.streamerId,
        table.moderatorId,
      ),
    };
...
export const moderatorPermissions = pgTable(
  'moderator_permissions',
  {
    relationshipId: uuid('relationship_id')
      .references(() => moderatorRelationships.id, { onDelete: 'cascade' })
      .notNull(),
  ...
  (table) => {
    return {
      relationshipPermissionIndex: uniqueIndex(
        'relationship_permission_idx',
      ).on(table.relationshipId, table.permissionKey),
    };
  },
);
...
export const remoteSessions = pgTable('remote_sessions', {
  ...
  relationshipId: uuid('relationship_id')
    .references(() => moderatorRelationships.id)
    .notNull(),
...
export const profiles = pgTable('profiles', {
  ...
  collaborationAvailability: boolean('collaboration_availability')
    .default(true)
    .notNull(),
...
export const posts = pgTable(
  ...
  (table) => {
    return {
      authorCreatedAtIdx: uniqueIndex('posts_author_created_at_idx').on(
        table.authorId,
        table.createdAt,
      ),
    };
...
export const reactions = pgTable(
  ...
  (table) => {
    return {
      userReactionTargetIdx: uniqueIndex('user_reaction_target_idx').on(
        table.userId,
        table.targetType,
        table.targetId,
        table.reactionType,
...
export const collaborations = pgTable(
  'collaborations',
  {
    ...
    applicationMode: varchar('application_mode', { length: 20 })
      .default('approval')
      .notNull(), // approval, open
    visibility: varchar('visibility', { length: 20 })
      .default('public')
      .notNull(), // public, private, invite_only
  ...
  (table) => {
    return {
      ownerStatusIdx: uniqueIndex('collab_owner_status_idx').on(
        table.ownerId,
        table.status,
      ),
    };
...
export const collaborationParticipants = pgTable(
  ...
  (table) => {
    return {
      collaborationUserIdx: uniqueIndex('collaboration_user_idx').on(
...
export const collaborationApplications = pgTable(
  ...
  (table) => {
    return {
      collabAppUserIdx: uniqueIndex('collab_app_user_idx').on(
        table.collaborationId,
        table.userId,
      ),
    };
...
export const collaborationInvitations = pgTable(
  ...
  (table) => {
    return {
      collabInviteUserIdx: uniqueIndex('collab_invite_user_idx').on(
        table.collaborationId,
        table.inviteeId,
      ),
    };
...
export const calendarEvents = pgTable(
  ...
  (table) => {
    return {
      ownerStartEndIdx: uniqueIndex('calendar_owner_start_end_idx').on(
        table.ownerId,
        table.startAt,
        table.endAt,
      ),
