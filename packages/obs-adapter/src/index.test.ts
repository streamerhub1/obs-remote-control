...
describe('ObsAdapter', () => {
    ...
    mockCall.mockImplementation((requestType) => {
      switch (requestType) {
        ...
        case 'GetSceneItemList':
          return Promise.resolve({
            sceneItems: [
              { sceneItemId: 1, sourceName: 'Mic', sceneItemEnabled: true },
            ],
          });
        case 'GetInputMute':
          return Promise.resolve({ inputMuted: false });
        case 'GetInputVolume':
          return Promise.resolve({ inputVolumeDb: -10, inputVolumeMul: 0.5 });
  ...
  it('should update state on ObsEvent', async () => {
    ...
    const eventHandler = mockOn.mock.calls.find(
      (call) => call[0] === 'CurrentProgramSceneChanged',
    )?.[1];
