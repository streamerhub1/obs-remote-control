    ...
    async (request, reply) => {
      ...
      return await db.transaction(async (tx) => {
        ...
        const [rel] = await tx
          ...
          .where(
            and(
              ...
              eq(moderatorRelationships.moderatorId, moderatorUserId),
            ),
          );
        if (!rel) {
          reply.status(404);
        ...
        if (presenceKeys.length === 0) {
          reply.status(400);
          throw new Error('Streamer is offline');
        }
        const streamerDeviceId = presenceKeys[0].split(':')[2];

        // Verify devices are not revoked
        const [modDev] = await tx
          .select()
          .from(devices)
          .where(eq(devices.id, moderatorDeviceId));
        if (!modDev || modDev.revokedAt) {
          reply.status(403);
          throw new Error('Moderator device is revoked or not found');
        }

        const [streamDev] = await tx
          .select()
          .from(devices)
          .where(eq(devices.id, streamerDeviceId));
        if (!streamDev || streamDev.revokedAt) {
          reply.status(403);
          throw new Error('Streamer device is revoked or not found');
        }
  ...
  server.get(
    ...
    async (request) => {
      ...
      const sessions = await db
        ...
        .where(
          or(
            eq(remoteSessions.streamerId, userId),
            eq(remoteSessions.moderatorId, userId),
          ),
        );
