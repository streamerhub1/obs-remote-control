...
import { users, devices, moderatorRelationships, moderatorPermissions, sessions } from '@obs-remote/database';
...
describe('Relay Transport & Moderator Flow Integration', () => {
  ...
  beforeAll(async () => {
    ...
    // Create Streamer
    const db = getDb();
    const [streamer] = await db.insert(users).values({
    ...
    const [sDev] = await db.insert(devices).values({
      userId: streamerId,
    ...
    const [mDev] = await db.insert(devices).values({
      userId: moderatorId,
    ...
    streamerToken = app.jwt.sign({ sub: streamerId, deviceId: streamerDeviceId });
    moderatorToken = app.jwt.sign({ sub: moderatorId, deviceId: moderatorDeviceId });
  });

  afterAll(async () => {
    if (!process.env.DATABASE_URL || !process.env.REDIS_URL) return;
    const db = getDb();
    await db.delete(moderatorPermissions).where(eq(moderatorPermissions.relationshipId, relationshipId));
    await db.delete(moderatorRelationships).where(eq(moderatorRelationships.id, relationshipId));
  ...
  it.skipIf(!process.env.DATABASE_URL || !process.env.REDIS_URL)('Complete Moderator Flow: Invite -> Accept -> Grant -> Connect -> Send Command', async () => {
    ...
    const redis = getRedis();
    await redis.setex(`presence:${streamerId}:${streamerDeviceId}`, 60, JSON.stringify({ online: true }));
