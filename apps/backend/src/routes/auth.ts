...
export default async function authRoutes(app: FastifyInstance) {
  ...
  server.get('/twitch/callback', async (request, reply) => {
    ...
    await db.transaction(async (tx) => {
      const [existingUser] = await tx.select({ id: users.id }).from(users).where(eq(users.twitchId, twitchUser.id));
      ...
      const [existingOauth] = await tx.select({ userId: oauthAccounts.userId }).from(oauthAccounts)
        .where(and(eq(oauthAccounts.provider, 'twitch'), eq(oauthAccounts.providerAccountId, twitchUser.id)));

      if (existingOauth) {
        await tx.update(oauthAccounts).set({
  ...
  server.post('/desktop/exchange', {
    ...
    const { code, publicKey, deviceName, platform, appVersion } = request.body;
  ...
  server.post('/desktop/challenge', {
    ...
    const db = getDb();
    const [device] = await db.select({ id: devices.id, revokedAt: devices.revokedAt }).from(devices).where(eq(devices.id, deviceId));
    
    if (!device) return reply.status(404).send({ error: 'Device not found' });
    if (device.revokedAt) return reply.status(403).send({ error: 'Device revoked' });
  ...
  server.post('/desktop/refresh', {
    ...
    return await db.transaction(async (tx) => {
      const [device] = await tx.select({ publicKey: devices.publicKey, revokedAt: devices.revokedAt, userId: devices.userId }).from(devices).where(eq(devices.id, deviceId));
      
      if (!device) return reply.status(404).send({ error: 'Device not found' });
      if (device.revokedAt) return reply.status(403).send({ error: 'Device revoked' });

      try {
        const isValid = crypto.verify(null, Buffer.from(challenge), device.publicKey, Buffer.from(signature, 'base64'));
        if (!isValid) {
      ...
      const [session] = await tx.select().from(sessions).where(eq(sessions.tokenHash, refreshTokenHash));
      ...
      if (session.revokedAt || session.replacedBySessionId) {
        ...
        await tx.update(sessions).set({ revokedAt: new Date() }).where(eq(sessions.familyId, session.familyId));
      ...
      await tx.update(devices).set({ lastSeenAt: new Date() }).where(eq(devices.id, deviceId));
  ...
  server.post('/logout', {
    ...
    if (refreshToken) {
      const refreshTokenHash = crypto.createHash('sha256').update(refreshToken).digest('hex');
      const db = getDb();
      await db.update(sessions).set({ revokedAt: new Date() }).where(eq(sessions.tokenHash, refreshTokenHash));
    }
