...
describe('Auth Routes Security Tests', () => {
  ...
  it('Device Identity: generate key pair, request challenge, sign, successful verify', async () => {
    ...
    mockDb.where.mockResolvedValueOnce([{ 
      id: 'session1',
      tokenHash: crypto.createHash('sha256').update('valid-refresh').digest('hex'),
      familyId: 'family1',
  ...
  it('Device Identity: modified challenge rejected', async () => {
    ...
    mockDb.where.mockResolvedValueOnce([{ publicKey: pubKeyPem, revokedAt: null, userId: 'user1' }]);

    const signature = crypto.sign(null, Buffer.from('fake-challenge'), privateKey).toString('base64');
  ...
  it('Device Identity: wrong key rejected', async () => {
    ...
    const signature = crypto.sign(null, Buffer.from('challenge'), otherPrivateKey).toString('base64');
  ...
  it('Device Identity: revoked device rejected', async () => {
    mockRedis.get.mockResolvedValueOnce('challenge');
    mockDb.where.mockResolvedValueOnce([{ publicKey: 'pub', revokedAt: new Date(), userId: 'user1' }]); // Revoked!
  ...
  it('Refresh Token: reuse old token revokes family', async () => {
    ...
    expect(response.statusCode).toBe(401);
    expect(JSON.parse(response.payload).error).toBe('Token reuse detected. Family revoked.');
