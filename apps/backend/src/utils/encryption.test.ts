...
describe('Encryption Utils', () => {
  ...
  it('should throw error if key is invalid length', () => {
    expect(() => encryptToken('test', INVALID_KEY)).toThrow(/must be exactly 32 bytes/);
    expect(() => decryptToken('test', INVALID_KEY)).toThrow(/must be exactly 32 bytes/);
  });

  it('should throw error if ciphertext format is invalid', () => {
    expect(() => decryptToken('invalid:format', TEST_KEY)).toThrow(/Invalid encrypted token format/);
    expect(() => decryptToken('v99:123:456:789', TEST_KEY)).toThrow(/Invalid encrypted token format/);
  });
  ...
  it('should fail to decrypt if cipher text is tampered', () => {
    ...
    parts[3] = parts[3].substring(0, parts[3].length - 1) + (parts[3].endsWith('0') ? '1' : '0');
    const tampered = parts.join(':');

    expect(() => decryptToken(tampered, TEST_KEY)).toThrow(/Unsupported state or unable to authenticate data/);
  });
