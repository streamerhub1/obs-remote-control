...
describe('Main process api', () => {
  ...
  it('validates feed list response structure', async () => {
    setupApiHandlers();
    const listHandler = (
      ipcMain.handle as unknown as { mock: { calls: unknown[][] } }
    ).mock.calls.find(
      (c: unknown[]) => c[0] === 'api:feed:list',
    )![1] as () => void;
    ...
    await expect(listHandler(null)).rejects.toThrow(
  ...
  it('api:feed:like validates response', async () => {
    ...
    await expect(likeHandler(null, 'post1')).rejects.toThrow(
      'Некорректный ответ сервиса',
    );
  });
});
