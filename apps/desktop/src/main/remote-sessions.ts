...
function loadDeviceIdentity() {
  try {
    const storePath = path.join(app.getPath('userData'), 'device_identity.json');
...
export function setupRemoteSessions() {
  ipcMain.handle('remoteSessions:connect', async (_, authorizationToken: string) => {
    return connectSession(authorizationToken);
  });

  ipcMain.handle('remoteSessions:disconnect', async (_, remoteSessionId: string) => {
  ...
  ipcMain.handle('remoteSessions:signChallenge', async (_, remoteSessionId: string, challengeHex: string) => {
    ...
    const signature = crypto.sign(null, Buffer.from(challengeHex, 'hex'), privateKey);
    return signature.toString('hex');
  });

  ipcMain.handle('remoteSessions:verifyProof', async (_, remoteSessionId: string, challengeHex: string, signatureHex: string, peerPublicKeyPem: string) => {
    const publicKey = crypto.createPublicKey(peerPublicKeyPem);
    const isValid = crypto.verify(null, Buffer.from(challengeHex, 'hex'), publicKey, Buffer.from(signatureHex, 'hex'));
    if (isValid) {
...
export async function connectSession(authorizationToken: string): Promise<SessionContext> {
  ...
  ws.on('message', (data) => {
    try {
      ...
      getMainWindow()?.webContents.send(`remoteSessions:message:${remoteSessionId}`, msg);
    } catch(e) {}
  });

  ws.on('close', () => {
    activeSessions.delete(remoteSessionId);
    getMainWindow()?.webContents.send(`remoteSessions:disconnected:${remoteSessionId}`);
  });
