...
const API = {
  ...
  signaling: {
    ...
    onDisconnected: (callback: () => void) => {
      ...
      return () =>
        ipcRenderer.removeListener('signaling:disconnected', callback);
    },
  },
  remoteSessions: {
    connect: (authorizationToken: string) =>
      ipcRenderer.invoke('remoteSessions:connect', authorizationToken),
    disconnect: (remoteSessionId: string) =>
      ipcRenderer.invoke('remoteSessions:disconnect', remoteSessionId),
    sendSignal: (remoteSessionId: string, msg: unknown) =>
      ipcRenderer.send('remoteSessions:sendSignal', remoteSessionId, msg),
    signChallenge: (remoteSessionId: string, challengeHex: string) =>
      ipcRenderer.invoke(
    ...
    executeCommand: (remoteSessionId: string, command: unknown) =>
      ipcRenderer.invoke(
    ...
    onIncoming: (callback: (session: unknown) => void) => {
      const handler = (_event: Electron.IpcRendererEvent, session: unknown) =>
        callback(session);
      ipcRenderer.on('remoteSessions:incoming', handler);
      return () =>
        ipcRenderer.removeListener('remoteSessions:incoming', handler);
    },
  },
  obs: {
  ...
  api: {
    ...
    collabs: {
      ...
      apply: (id: string, message?: string) =>
        ipcRenderer.invoke('api:collabs:apply', id, message),
    ...
    profile: {
      ...
      updateMe: (data: unknown) =>
        ipcRenderer.invoke('api:profile:updateMe', data),
    },
    notifications: {
      list: () => ipcRenderer.invoke('api:notifications:list'),
      markAllRead: () => ipcRenderer.invoke('api:notifications:markAllRead'),
      markRead: (id: string) =>
        ipcRenderer.invoke('api:notifications:markRead', id),
    },
    relationships: {
      list: () => ipcRenderer.invoke('api:relationships:list'),
      invite: (data: unknown) =>
        ipcRenderer.invoke('api:relationships:invite', data),
      respond: (id: string, data: unknown) =>
        ipcRenderer.invoke('api:relationships:respond', id, data),
      revoke: (id: string) =>
        ipcRenderer.invoke('api:relationships:revoke', id),
      getPermissions: (id: string) =>
        ipcRenderer.invoke('api:relationships:getPermissions', id),
      setPermissions: (id: string, data: unknown) =>
  ...
  updater: {
    ...
    onStateChanged: (callback: (state: string, data?: unknown) => void) => {
      const handler = (
        _event: Electron.IpcRendererEvent,
        payload: { state: string; data?: unknown },
