    ...
    <div className="space-y-6">
      <div className="flex flex-wrap gap-4">
        <div className="bg-[#161616] border border-gray-800 rounded-xl p-4 flex-1 min-w-[200px]">
          <h4 className="text-gray-500 text-xs mb-1 uppercase tracking-wide">
            Версия
          </h4>
          <p className="font-medium text-sm">
            {snapshot.obsVersion} / WS {snapshot.websocketVersion}
          </p>
        </div>
        <div className="bg-[#161616] border border-gray-800 rounded-xl p-4 flex-1 min-w-[200px]">
          <h4 className="text-gray-500 text-xs mb-1 uppercase tracking-wide flex justify-between">
            Трансляция
            {snapshot.streamStatus?.active ? (
              <PlaySquare size={14} className="text-red-400" />
            ) : (
              <Square size={14} />
            )}
          </h4>
          <p
            className={cn(
              'font-medium',
              snapshot.streamStatus?.active ? 'text-red-400' : 'text-gray-300',
            )}
          >
            {snapshot.streamStatus?.active
              ? `В эфире: ${snapshot.streamStatus.timecode}`
              : 'Остановлена'}
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-[#161616] border border-gray-800 rounded-xl p-6 shadow-lg flex flex-col h-[400px]">
          <h3 className="text-lg font-medium flex items-center gap-2 mb-4">
            <Video size={20} className="text-blue-400" /> Сцены
          </h3>
                  ...
                  <button
                    ...
                    className={cn(
                      ...
                      isActive
                        ...
                        : 'bg-black border-gray-800 text-gray-400 hover:border-gray-600 hover:text-gray-200',
                    )}
                  >
                    {sceneName}
                  </button>
        ...
        <div className="bg-[#161616] border border-gray-800 rounded-xl p-6 shadow-lg flex flex-col h-[400px]">
          <h3 className="text-lg font-medium flex items-center gap-2 mb-4">
            <Layers size={20} className="text-purple-400" /> Источники (
                    ...
                    <button
                      onClick={() =>
                        execute({
                          ...
                          payload: {
                            sceneName: snapshot.currentProgramScene,
                      ...
                      className={cn(
                        'p-1.5 rounded-md hover:bg-gray-800 transition-colors',
