...
import { UserPlus, Heart, MessageCircle, Calendar as CalendarIcon, Check, Loader2, Bell } from 'lucide-react';
...
export function Notifications() {
  ...
  const getMessage = (n: Notification) => {
    switch (n.type) {
      ...
      case 'collab_application': return <span><strong className="text-white">{n.actor.displayName}</strong> подал заявку на вашу коллаборацию</span>;
      case 'collab_accepted': return <span>Ваша заявка на коллаборацию принята!</span>;
    ...
    <div className="space-y-6 max-w-3xl mx-auto">
      <header className="flex justify-between items-end">
        <div>
          <h2 className="text-3xl font-semibold text-gray-100">
            Уведомления
            {unreadCount > 0 && (
              <span className="ml-3 text-base font-normal bg-blue-600 text-white rounded-full px-2.5 py-0.5">{unreadCount}</span>
            )}
          </h2>
        </div>
        {unreadCount > 0 && (
          <Button variant="outline" size="sm" className="text-gray-400 border-gray-800" onClick={markAllRead}>
      ...
      {loading && (
        <div className="flex items-center justify-center py-16 text-gray-500">
          <Loader2 className="w-6 h-6 animate-spin mr-3" /> Загрузка уведомлений...
        </div>
      )}
      {error && !loading && (
        <div className="bg-red-900/20 border border-red-800/40 rounded-xl p-6 text-center">
          ...
          <Button variant="outline" onClick={fetchNotifications}>Повторить</Button>
        </div>
      )}
      {!loading && !error && notifications.length === 0 && (
        <div className="bg-[#161616] border border-gray-800 rounded-xl p-12 text-center">
          ...
          <p className="text-gray-600 text-sm mt-2">Здесь будут появляться лайки, подписки и приглашения.</p>
        </div>
      )}
      ...
      {!loading && (
        <div className="space-y-3">
          {notifications.map(n => (
            <Card
              key={n.id}
              className={cn('border-gray-800 transition-colors cursor-pointer', !n.readAt ? 'bg-[#1A1A1A] border-l-2 border-l-blue-500' : 'bg-[#161616]')}
              onClick={() => !n.readAt && markRead(n.id)}
            >
              <CardContent className="p-4 flex items-center gap-4">
                <div className="p-2 bg-black/30 rounded-full flex-shrink-0">{getIcon(n.type)}</div>
                <Avatar className="w-10 h-10 flex-shrink-0" src={n.actor.avatarUrl ?? undefined} fallback={n.actor.displayName[0]} />
                ...
                {!n.readAt && <div className="w-2 h-2 rounded-full bg-blue-500 flex-shrink-0" />}
              </CardContent>
            </Card>
          ))}
        </div>
