...
import { Calendar as CalendarIcon, Clock, Search, Loader2, Plus, Users } from 'lucide-react';
...
export function Collabs() {
  ...
  React.useEffect(() => { fetchCollabs(); }, [fetchCollabs]);
    ...
    <div className="space-y-6">
      <header className="flex justify-between items-end">
        <div>
          ...
          <p className="text-gray-400 mt-1">Ищите партнеров для совместных стримов.</p>
        </div>
        <Button onClick={() => setShowCreate(!showCreate)}>
          <Plus className="w-4 h-4 mr-2" />Создать коллаборацию
        </Button>
      </header>
      ...
      {showCreate && (
        <div className="bg-[#161616] border border-blue-800/40 rounded-xl p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-xs text-gray-400 mb-1">Макс. участников</label>
          ...
          <div className="flex gap-3 justify-end">
            ...
            <Button onClick={handleCreate} disabled={creating || !newTitle.trim() || !newDate}>
      ...
      <div className="flex-1 bg-black/50 border border-gray-800 rounded-lg flex items-center px-3">
        ...
        <input type="text" placeholder="Поиск коллабораций..." value={search} onChange={e => setSearch(e.target.value)}
      ...
      {!loading && !error && filtered.length === 0 && (
        <div className="bg-[#161616] border border-gray-800 rounded-xl p-12 text-center">
          ...
          <p className="text-gray-600 text-sm mt-2">Создайте первую или дождитесь приглашений от других стримеров.</p>
        </div>
      )}
            ...
            <Card key={collab.id} className="bg-[#161616] border-gray-800 hover:border-gray-700 transition-colors flex flex-col">
              ...
              <CardContent className="flex-1 space-y-3">
                <div className="flex items-center gap-3">
                  <Avatar className="w-8 h-8" src={collab.host.avatarUrl ?? undefined} fallback={collab.host.displayName[0]} />
                  <span className="text-sm font-medium text-gray-300">{collab.host.displayName}</span>
                </div>
                <div className="space-y-1 text-sm text-gray-400">
                  <div className="flex items-center gap-2">
                    <CalendarIcon className="w-4 h-4" />
                    <span>{new Date(collab.startAt).toLocaleDateString('ru', { day: 'numeric', month: 'long', hour: '2-digit', minute: '2-digit' })}</span>
                  </div>
                  <div className="flex items-center gap-2">
              ...
              <CardFooter className="border-t border-gray-800/50 pt-4">
                {collab.myApplication === 'pending' ? (
                  <div className="w-full text-center text-sm text-yellow-400 py-2">Заявка отправлена</div>
                ) : collab.myApplication === 'accepted' ? (
                  <div className="w-full text-center text-sm text-green-400 py-2">Заявка принята ✓</div>
                ) : (
                  <Button className="w-full" onClick={() => handleApply(collab.id)}
                    disabled={applying === collab.id || collab.currentParticipants >= collab.maximumParticipants}>
                    {applying === collab.id ? <Loader2 className="w-4 h-4 mr-2 animate-spin" /> : null}
                    Подать заявку
                  </Button>
                )}
              </CardFooter>
