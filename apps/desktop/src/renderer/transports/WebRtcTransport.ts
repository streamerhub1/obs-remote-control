import {
  ...
  createP2PMessage,
  P2PPayload,
} from '@obs-remote/remote-protocol';
...
export class WebRTCManager {
  ...
  public async connect() {
    ...
    this.pc.onicecandidate = (event) => {
      if (event.candidate) {
        window.desktop.remoteSessions.sendSignal(this.remoteSessionId, {
          type: 'signaling.ice',
    ...
    this.pc.onconnectionstatechange = () => {
      ...
      if (this.pc?.connectionState === 'connected') {
        ...
        if (this.role === 'moderator') {
          ...
          this.sendP2PMessage('handshake.hello', {
            appVersion: '1.0.0',
            deviceId: myDeviceId,
          });
        }
  ...
  private async handleP2PMessage(msg: P2PMessage, channel: string) {
    if (msg.type === 'heartbeat.ping') {
      this.sendP2PMessage('heartbeat.pong', {
      ...
      msg.type === 'handshake.challenge' &&
      this.role === 'moderator'
    ) {
      const payload = msg.payload as Extract<
        P2PPayload,
        { type: 'handshake.challenge' }
      >['payload'];
      try {
        const signatureHex = await window.desktop.remoteSessions.signChallenge(
          this.remoteSessionId,
          payload.challenge,
        );
      ...
      try {
        ...
        const valid = await window.desktop.remoteSessions.verifyProof(
          this.remoteSessionId,
          'mock',
          payload.proof,
          'mock',
      ...
      window.dispatchEvent(
        new CustomEvent('obs:remoteSnapshot', { detail: payload.snapshot }),
      );
      ...
      if (payload.revision !== this.currentRevision + 1) {
        this.sendP2PMessage('state.resyncRequest', {
      ...
      this.currentRevision = payload.revision;
      window.dispatchEvent(
        new CustomEvent('obs:remoteEvent', { detail: payload.event }),
      );
    }
  }
  ...
  public sendP2PMessage(type: P2PPayload['type'], payload: unknown) {
    if (!this.controlChannel || this.controlChannel.readyState !== 'open')
      return;
    this.sequenceCounter++;
    const msg = createP2PMessage(
      this.remoteSessionId,
      this.sequenceCounter,
      type,
      payload,
  ...
  public broadcastEvent(event: unknown) {
    if (this.role === 'streamer') {
      this.currentRevision++;
      this.sendP2PMessage('state.patch', {
        revision: this.currentRevision,
        event,
      });
    }
