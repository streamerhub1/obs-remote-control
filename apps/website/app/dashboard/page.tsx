...
export default function DashboardOverview() {
  const [user, setUser] = useState<{ displayName: string, avatarUrl: string, inviteCode: string } | null>(null);
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <header className="flex justify-between items-end pb-6 border-b border-[#222]">
        <div>
          <h1 className="text-3xl font-bold text-white tracking-tight">Добро пожаловать, {user?.displayName || 'Стример'}</h1>
          <p className="text-gray-400 mt-2">Управляйте подключениями и модераторами из единого центра.</p>
        </div>
        {user && (
          <img src={user.avatarUrl} alt="Avatar" className="w-16 h-16 rounded-full border-2 border-purple-500" />
        )}
      </header>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-[#111] border border-[#222] p-6 rounded-2xl shadow-xl hover:border-purple-500/50 transition-colors group">
          ...
          <p className="text-gray-400 text-sm">Подключено 1 устройство (OBS Desktop)</p>
        </div>
        ...
        <div className="bg-[#111] border border-[#222] p-6 rounded-2xl shadow-xl hover:border-green-500/50 transition-colors group">
          ...
          <div className="flex items-center gap-2 mt-1">
            ...
            <span className="text-gray-400 text-sm">Система работает стабильно</span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-[#111] border border-[#222] rounded-2xl p-8 shadow-xl mt-8">
        ...
        <p className="text-gray-400 mb-6">Отправьте этот код модератору, чтобы он мог отправить вам запрос на подключение.</p>
