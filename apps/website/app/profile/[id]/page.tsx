      ...
      <main className="max-w-5xl mx-auto px-6 py-12 space-y-8">
        <div className="relative">
          ...
          <div className="absolute -bottom-16 left-8 flex items-end gap-6">
            <Avatar
              className="w-32 h-32 border-4 border-[#0A0A0A] bg-[#161616]"
              src={profile.avatarUrl}
              fallback={profile.displayName[0]}
            />
          </div>
          <div className="absolute top-4 right-4 flex gap-2">
            <Button className="bg-white text-black hover:bg-gray-200 shadow-xl shadow-white/10">
              Подписаться
            </Button>
            <Button
              variant="secondary"
              className="bg-black/50 hover:bg-black/70 border-none backdrop-blur-md"
            >
              Предложить коллаб
            </Button>
          </div>
        ...
        <div className="pt-20 px-8">
          ...
          <div className="mt-8 flex flex-wrap gap-2">
            {profile.categories.map((cat) => (
              <Badge key={cat} variant="secondary" className="text-sm py-1">
                {cat}
              </Badge>
            ))}
          </div>
            ...
            <Card className="bg-[#161616] border-gray-800 hover:border-gray-700 transition-colors">
              <CardHeader>
                ...
                <CardTitle className="text-lg line-clamp-2 leading-tight">
                  Valheim Boss Rush: Next Weekend
                </CardTitle>
              </CardHeader>
